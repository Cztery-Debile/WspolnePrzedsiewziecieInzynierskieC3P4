...
# Funkcja obsługująca zaznaczanie obszarów
...
# Funkcja usuwająca ostatnio dodany obszar
...
while True:
    ...
    if key == ord('q'):
        ...
        for area in selected_areas:
            ...
            result_image = cropped_region.copy()  # Utwórz nowy obraz na którym będą rysowane wyniki detekcji
            for result in results:
                boxes = result.boxes.cpu().numpy()
                for box in boxes:
                    class_id = int(box.cls[0])  # klasa boxa
                    confidence = box.conf.astype(float)
                    if class_id == 0:  # określenie klasy dla tego modelu 0->person 1->head
                        r = box.xyxy[0].astype(int)
                        class_name = model.names[class_id]
                        x_center = (r[0] + r[2]) // 2
                        y_center = (r[1] + r[3]) // 2
                        cv2.rectangle(result_image, r[:2], r[2:], (0, 255, 0), 2)  # rysowanie na obrazie boxa
                        cv2.putText(result_image, f'Name: {class_name}', (x_center, y_center), cv2.FONT_HERSHEY_SIMPLEX,
                                    0.4, (0, 255, 0), 1)
                        cv2.putText(result_image, f'{confidence}', (x_center, y_center + 20), cv2.FONT_HERSHEY_SIMPLEX,
                                    0.4, (255, 0, 0), 1)
                    ...
                    if class_id == 1:
                        ...
                        x_center = (r[0] + r[2]) // 2
                        y_center = (r[1] + r[3]) // 2
                        cv2.rectangle(result_image, r[:2], r[2:], (0, 0, 255), 2)  # rysowanie na obrazie boxa
                        cv2.putText(result_image, f'Name: {class_name}', (x_center, y_center), cv2.FONT_HERSHEY_SIMPLEX,
                                    0.5, (0, 255, 0), 2)
                        cv2.putText(result_image, f'{confidence}', (x_center, y_center + 20), cv2.FONT_HERSHEY_SIMPLEX,
                                    0.4, (255, 0, 0), 1)

            # Wyświetl przetworzony obraz w osobnym oknie
...
# Wyświetlenie wyników detekcji w osobnym oknie
for result in detected_results:
    result.show()

# Zamykanie wszystkich okien po naciśnięciu klawisza
cv2.waitKey(0)
